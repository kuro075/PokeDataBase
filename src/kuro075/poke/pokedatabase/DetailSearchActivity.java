...
public class DetailSearchActivity extends BookMenuActivity{
	/**
	 * 検索条件を取得した時のリスナー
	 * @author sanogenma
	 *
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//画面上部
		final int length=getNumOfSearchableInformations();
		...
		for(int i=0;i<length;i++){
			...
			button_add_if[i].setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					openFilterDialog(index,new MySearchIfListener(context,index));
					//PokeSearchableInformations.values()[index].openDialog(context, SearchTypes.FILTER, );
				}
			});
			//条件テキスト
		...
		button_search.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				startSearchResultActivity(list.toArray(new String[0]));
	...
	/**
	 * SearchableInformationsのタイトルを取得
	 * @param index
	 * @return
	 */
	...
	protected void openFilterDialog(int index,SearchIfListener listener){
		Utility.log(TAG, "openFilterDialog");
	}
	...
	private void refreshIfName(int index){
		StringBuilder sb=new StringBuilder();
		sb.append(getSearchableInformationTitle(index));
