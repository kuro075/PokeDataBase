...
public class OtherInformationLayout extends FrameLayout{
	...
	protected OtherInformationLayout(Context context,OnTouchListener otl,PokeData poke) {
		super(context);
    ...
    private void clickEvolutionName(View v){
    	...
    	/*for(int i=0;i<textEvolutions.length;i++){
    		if(textEvolutions[i]==v){
    			Intent intent = new Intent(InformationOfPokeActivity.this,InformationOfPokeActivity.class);
			    intent.putExtra(Utility.EXTRA_NAME, textEvolutions[i].getText().toString());
		        startActivityForResult(intent,Utility.RESULT_ID_INFOPOKE);
    		}
    	}*/
    }
